...
congresistas = pd.read_csv(f'{ruta_actual}\\data\\Congresistas_raw.csv', sep=';')
proyectos = pd.read_csv(f'{ruta_actual}\\data\\Proyectos_raw.csv', sep=';')
print(congresistas.head())
Proyectos_completo = pd.merge(congresistas, proyectos, on='ID_CONGRESISTA')

Proyectos_completo.to_csv(f'{ruta_actual}\\data\\Datos_proyectos_procesado.csv', index=False)
